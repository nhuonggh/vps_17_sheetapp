 ...
 * ========================================
 * Main webhook endpoint để nhận PayOS payment notifications
 * 
 * Dependencies:
 * - Config.gs
 * - SupabaseClient.gs (findOrderById, supabaseUpdate, isTransactionProcessed)
 * - AutoEnrollment.gs (enrollUserInProducts, logFailedEnrollment)
 * - TransactionLogger.gs (logTransaction)
 * - GoogleChatHelper.gs (gchat_newbug)
 */
...
function doPost(e) {
  ...
  try {
    ...
    gchat_newbug("PayOS Webhook", "Received", JSON.stringify(webhookData));
    ...
    // Send error alert
    gchat_newbug('PayOS Webhook', 'Error', `❌ ${error.message}`);
...
function processPayOSWebhook(webhookData) {
  ...
  const message = 
    ...
    `🏦 Ngân hàng: ${data.counterAccountBankName || data.counterAccountBankId}\\n` +
