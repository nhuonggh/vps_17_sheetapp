...
export default function ConsultationModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Verify CAPTCHA to prevent bot spam
      if (!executeRecaptcha) {
      ...
      const captchaToken = await executeRecaptcha('consultation');
      ...
      if (!captchaResponse.ok || !captchaData.success) {
        alert('Bot detected! Please try again or contact us directly via phone.');
        setLoading(false);
        return;
      }
