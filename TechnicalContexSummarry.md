...
Người dùng truy cập: Xem danh sách khóa học/dịch vụ từ trang chủ
Tìm kiếm & Lọc: Sử dụng bộ lọc theo ngành nghề, công nghệ, giá cả
Xem chi tiết sản phẩm: Chi tiết khóa học (chapters, lessons) hoặc dịch vụ (features, quy trình)
Thêm vào giỏ hàng: Cart context lưu localStorage
Đăng nhập/Đăng ký: OAuth Google qua Supabase Auth
Thanh toán: Hiển thị QR banking (chưa tích hợp gateway)
Học tập: Truy cập bài học sau khi mua (đang xây dựng)
...
6.1 Core Features
 ...
 Product Catalog: Listing, Detail, Categories
 Cart System: Add, Remove, localStorage persistence
 Responsive Layout: Mobile-first, Desktop optimized
 SEO: Meta tags, JSON-LD structured data
 Search & Filter: Keyword, Cost, Industry, Tech tags
6.2 Pages
 Homepage (Hero, Courses, Services, News, Testimonials, Partners)
 Categories (Mobile: Grouped tabs + filter drawer, PC: Sidebar filters)
 Product Detail (Tabs: Content, Instructor, Reviews, Process)
 ...
 404 Not Found
6.3 Components
 Navbar (Desktop, 2-level dropdown menu)
 MobileHeader (Search, Cart badge, Menu drawer)
 MobileBottomNav (4 tabs: Home, Categories, Cart, Profile)
 Footer (Company info, Social links, Payment QR)
 FloatingContact (Zalo, Messenger, Phone buttons)
 CourseTabs (Tabbed interface for product details)
 ProductCard, ProductActions, ProductImageSlider
 BookingModal, ConsultationModal
...
4-tier tab system:
Parent tabs: Tất cả / Khóa học / Dịch vụ
Child tabs: Horizontal scroll pills (Online, Zoom, AppSheet...)
Filter drawer: Full-screen slide-in từ right
...
Check responsive: Test trên mobile và desktop
Update constants.ts: Nếu có config mới
TypeScript types: Luôn định nghĩa interface
Error handling: Xử lý null/undefined cases
SEO: Thêm metadata cho pages mới
Khi sửa bug:
Console logs: Tránh để lại console.log trong production
ESLint warnings: Fix hoặc comment // eslint-disable-next-line
Build test: Chạy npm run build trước khi commit
Database migrations:
